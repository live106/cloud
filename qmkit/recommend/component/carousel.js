...
import {StyleSheet, Text, ScrollView, View, InteractionManager, Platform} from 'react-native';
...
const Carousel = React.createClass({
  ...
  _onScrollEnd: function (event) {
    ...
    if (__DEV__) {
      console.log("_onScrollEnd is called ", offset.x, offset);
    }
  ...
  _setUpTimer: function () {
    if (__DEV__) {
      console.log('_setUpTimer is called ', this.props.autoplay, this.props.children.length);
    }
    //only for cycling
  ...
  _animateNextPage: function () {
    ...
    if (__DEV__) {
      console.log('_animateNextPage is called ', k, size);
    }
    k++;
    ...
    this.refs.scrollView.scrollTo({x: k * size.width, y: 0, animated: true});
    this._setUpTimer();
  },
    ...
    this.setState({currentPage: page}, () => {
      ...
      if (this.props.onChangeTab) {
        InteractionManager.runAfterInteractions(() => {
          if (__DEV__) {
            console.log("*** recommend on change tab ", page);
          }
          this.props.onChangeTab({i: page > 0 ? page - 1 : page});
        });
      }
    });
  },
