...
import {
  StyleSheet, BackHandler, InteractionManager, StatusBar, Platform, AsyncStorage
} from 'react-native';
//上一次点击Android的返回键的时间
...
const {unAuthMap, unAuth} = QMUnAuth;

import Login from './login/index';
import Home from './home';
import EpubReader from './epub-reader/index';
import Map from './map/index';
import RelatedResource from './related-resource/index';
...
class AppRoute extends React.Component {
    ...
    return (
      <Navigator
        ...
        renderScene={(route, navigator) => {
          ...
          if (Platform.OS === 'ios') {
            if (sceneName === 'Home' && (window.selectedTab === undefined || window.selectedTab === 'mainPage')) {
              msg.emit('index:setStatusBarStyle', 'light-content');
            } else {
              StatusBar.setBarStyle('default');
            }
          }

          let Scene = routeMapper[sceneName];

          if (!unAuthMap[sceneName] && !window.id) {
  ...
  _handleBackAndroid() {
    ...
    //实体键关闭弹框
    msg.emit('app:setAlertV isible', false);
    //android物理回退键是否应该响应
  ...
  async _loginAtOnce() {
    ...
    await AsyncStorage.setItem('kstore@showGuide', '{"showGuide":' + JSON.stringify(`${QMConfig.VERSION}`) + '}');
  }
